...
Nine ordinary packages plus optional `net`; one busybox-style C99 binary dispatched on `--tool <id>`; each package ships its own copy. Zero special treatment.
...
### 8.8 code and project (A3)

- code.read-range {path(read,must_exist), start_line?=1, end_line?=200} → {content,start_line,end_line,total_lines,truncated} [read-only,idempotent]. Inclusive 1-based range, maximum 1000 lines and 16 MiB source file; no process permission.
- code.search-symbol {symbol(identifier), path?(read)=".", glob?, max_results?=100} → the grep match/coverage shape [read-only,idempotent]. The caller cannot supply a regex; the implementation constructs a whole-identifier expression.
- code.apply-patch {patch, strip?=1} → {files_changed,hunks_applied} [destructive]. Process-free alias over the same atomic, contained implementation as edit.patch.
- project.build / test / lint / format / diagnostics {path?(rw)=".", adapter?∈{auto,cmake,cargo,npm,python}="auto"} → {adapter,exit_code,steps:[{program,exit_code,stdout,stderr,stdout_truncated,stderr_truncated}]} [destructive,long-running, timeout PT5M]. Marker-based auto detection; all executable names and argv are constants in astools-std. The manifest requests `proc`, but grants are per tool: granting `project` never enables `proc.run`.
...
`astools-mcp --root <dir> [--config <file>] [--workspace <dir>]`; stdio JSON-RPC 2.0. Supports stateless MCP `2026-07-28` (`server/discover`, per-request protocol `_meta`) and legacy `2025-06-18` (`initialize`/`initialized`); other versions fail with `-32022`. Tool naming `<tool>_<command>` (bijective, D7). tools/list: composed description, generated inputSchema, annotations readOnlyHint/destructiveHint/idempotentHint/openWorldHint(net). tools/call → astools_invoke; tool-level failure ⇒ result isError:true with code+message; engine failures ⇒ JSON-RPC errors with astools_err name in data. Management tools only when mcp.management: astools_registry_refresh, astools_tool_manifest, astools_sandbox_caps, astools_stats. In-house strict RFC 8259 JSON (UTF-8 only, depth-capped). Plugin behaviors (A2 §A2.3.4): 1) missing --config file ⇒ builtin defaults + info log (never create); `--config-init` writes commented default template and exits. 2) workspace resolution: --workspace ▷ ASTOOLS_WORKSPACE ▷ config workspace.root ▷ cwd. 3) installation registry root: `<exedir>/../share/astools/tools` (POSIX) / `<exedir>\tools` (Windows) prepended at trust standard when it exists. Precedence: install root ▷ ASTOOLS_PATH ▷ registry.paths.

## 12. Configuration `#astools_config` (all keys optional; defaults shown)
registry: { paths: [{path:"tools", trust:"standard"}], watch:"poll", poll_interval:r"PT5S", pinning:"warn", lock_path:"astools.lock.xcdn", pins:[] } · workspace: { root:".", scratch:null } · sandbox: { default_level:"basic", strict_fallback:"degrade", allow_library:false, executable_paths:[] } · grants: { workspace_access:"read-write", tools:[] } · invocation: { timeout:r"PT30S", max_concurrent:4, max_output_bytes:1048576, stderr_max_bytes:262144, result_validation:"warn" } · catalog: { level:"summary", char_budget:8000, priority:[] } · mcp: { management:false } · logging: { path:null, level:"info", max_size_kb:5120, max_files:3, sync:false, audit:false }. `sandbox.executable_paths` is an operator-controlled array of absolute directories appended to the otherwise fixed scrubbed child `PATH`; the host `PATH` is never inherited implicitly.

## 14. Platform notes
POSIX first. Atomic replace only rename primitive. LF written (CRLF accepted). All astools-controlled names ASCII lowercase. macOS Seatbelt via astools-jail (deprecated API, functional); RLIMIT_AS unreliable on macOS (best effort). Windows: implemented (os_proc_win32.c / sandbox_win32.c) — every spawn in its own Job Object (kill-tree, cpu/mem/active-process caps), UTF-16 conversions throughout, overlapped stdin pipe for nonblocking writes; strict-level enforcement (restricted tokens, full fs confinement, net deny) remains deferred and degrades to basic. Canonical paths are '/'-separated on every platform (os_realpath folds '\\'; path containment is case-insensitive and accepts drive-rooted "X:/…" and UNC "//srv/…" forms). The std suite runs on Windows through std/compat_win32.{h,c}; final-component O_NOFOLLOW uses FILE_FLAG_OPEN_REPARSE_POINT plus handle validation. Parent-component confinement is still enforced by preflight rather than a restricted token, so basic remains process isolation rather than a strict filesystem sandbox. The scrubbed sandbox PATH includes %ProgramFiles%\Git\cmd when present (the Windows counterpart of /usr/bin) and then the explicit `sandbox.executable_paths`. The portable CTest suite configures and runs on Windows; POSIX-only symlink containment cases remain in the Linux/macOS jobs.
